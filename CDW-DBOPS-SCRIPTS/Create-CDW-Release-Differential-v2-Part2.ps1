...
#These values should match what is run in with Part 1 script.
...
$dbops_release_branch = $( 'ReleaseNew_from_' + $start_tag.ToUpper().replace('RELEASE_','') + '_to_' + $end_tag.ToUpper().Replace('RELEASE_','') ) #handle case-sensitive.
...
if( -not (Test-Path($path_dbops+'\'+$dbops_release_folder)))
{
    Write-host "Can not find Release folder. Was Differential Part 1 script ever run?" -ForegroundColor Red
    Write-Host "If so, was the inital PR merged into [main] in GitHub?"
    exit
}
...
#$diff_hash = Build-GitDiffHash -StartTag $start_tag -EndTag $end_tag -MainBranch $cdw_main -FileList $diff_files
...
#DO NOT CLEAR THE SUB FOLDER. OVERWRITE ITS CONTENTS!!!
...
#We're now handling the Current Release
Set-location $path_cdw
...
git checkout $cdw_main #Reattached to HEAD.
